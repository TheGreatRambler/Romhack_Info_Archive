...
if(fs.existsSync("config.json")) {
	...
	config
		= CommentJSON.parse(fs.readFileSync("./default_config.json").toString())
}
					...
					try {
						...
						if(hackEntry) {
							...
							if(hackEntry.release) {
								hackEntry.release
									= moment(hackEntry.release, dateFormat);
...
async function pokemonArchive1() {
	...
	await handleWebpageTemplate(links, async function returnHackEntry(page) {
		return page.evaluate(() => {
			var system = document.getElementsByClassName("entry-categories")[0]
							 .firstElementChild.innerText.split(" ")[0];
			return {
				name: document.getElementsByTagName("h1")[0].innerText,
					...
					originalgame:
						document.getElementsByTagName("td")[6].innerText,
					system: system, downloads: null, type: null,
					// We can't know
					important: false, source: "pokemonromhack list"
			}
		});
	}, "html", "YYYY");
...
async function generalArchive1() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.romhacking.net/?page=hacks&perpage=200&startpage="
				+ start,
			{ waitUntil: "domcontentloaded", timeout: 300000 });

		var partialLinks = await mainBrowserPage.evaluate(() => {
			var allLinks
				= Array.from(document.getElementsByClassName("col_1 Title"),
					a => a.firstElementChild.href);
		...
		return page.evaluate(() => {
			...
			var temp = {
				name: document.getElementById("main")
						  ...
						  .innerText,
				author: document.getElementsByTagName("td")[1]
							.firstElementChild.innerText,
				release: document.getElementsByTagName("td")[8].innerText,
				originalgame:
					document.getElementsByTagName("h4")[0].innerText.replace(
						"Hack of ", ""),
				system: document.getElementsByTagName("td")[3]
							.firstElementChild.innerText,
...
async function smwArchive1() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.smwcentral.net/?p=section&s=smwhacks&n=" + start,
			{ waitUntil: "domcontentloaded", timeout: 300000 });

		var partialLinks = await mainBrowserPage.evaluate(() => {
			return Array.from(document.getElementsByClassName("gray small"),
				a => a.previousElementSibling.previousElementSibling.href);
		});
	...
	await handleWebpageTemplate(links, async function returnHackEntry(page) {
		return page.evaluate(() => {
			var temp = {
				...
				release:
					document.getElementsByClassName("cell2")[1].innerText.split(
						" ")[0],
				originalgame: "Super Mario World",
				system: "SNES",
				downloads: document.getElementsByClassName("small")[0]
							   .innerText.split(" ")[0]
							   .replace(/\,/g, ""),
				source: "smwcentral.net smw"
			}

			if(document.getElementsByClassName("cell1")[3].innerText
				=== "Version History:") {
				temp.type = document.getElementsByClassName("cell2")[7]
								.innerText;
				temp.author
					= document.getElementsByClassName("cell2")[3].innerText;
			}
			else {
				temp.type = document.getElementsByClassName("cell2")[6]
								.innerText;
				temp.author
					= document.getElementsByClassName("cell2")[2].innerText;
			}
...
async function sm64Archive1() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.smwcentral.net/?p=section&s=sm64hacks&n=" + start,
			{ waitUntil: "domcontentloaded", timeout: 300000 });
	...
	await handleWebpageTemplate(links, async function returnHackEntry(page) {
		return page.evaluate(() => {
			...
			if(document.getElementsByClassName("cell1")[3].innerText
				=== "Version History:") {
				temp.type = document.getElementsByClassName("cell2")[4]
								.innerText
							+ " "
							+ document.getElementsByClassName("cell2")[6]
								  .innerText;
				temp.author
					= document.getElementsByClassName("cell2")[3].innerText;
			}
			else {
				temp.type = document.getElementsByClassName("cell2")[5]
								.innerText
							+ " "
							+ document.getElementsByClassName("cell2")[7]
								  .innerText;
				temp.author
					= document.getElementsByClassName("cell2")[2].innerText;
			}
...
async function yoshisIslandArchive1() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.smwcentral.net/?p=section&s=yihacks&n=" + start,
			{ waitUntil: "domcontentloaded", timeout: 300000 });
...
async function sm64Archive2() {
	...
	var links = await mainBrowserPage.evaluate(() => {
		...
		for(var tables = 0; tables < 4; tables++) {
			allLinks = allLinks.concat(
				Array
					.from(document.getElementsByTagName("table")[tables]
					...
					.map(element => element.firstElementChild.firstElementChild
										.href));
		}
				...
				if(haveReachedPoint) {
					...
					if(element.firstElementChild
						&& element.firstElementChild.id
							   === "Hacks_to_be_added_to_this_list:") {
	...
	await handleWebpageTemplate(links, async function returnHackEntry(page) {
		var shouldInclude = await page.evaluate(() => {
			return document.getElementsByClassName("pi-data-value pi-font")
					   .length
				   != 0;
		});
			...
			return page.evaluate(() => {
				...
				if(document
						.getElementsByClassName(
							"pi-data-label pi-secondary-font")[1]
						.innerText
					=== "Published") {
					temp.release = document
									   .getElementsByClassName(
										   "pi-data-value pi-font")[1]
									   .innerText;
					temp.important
						= parseInt(document
									   .getElementsByClassName(
										   "pi-data-value pi-font")[2]
									   .innerText)
						  >= 70;
				} else {
					temp.release = null;
...
async function sm64DSArchive1() {
	...
	await mainBrowserPage.goto(
		"https://sm64dsromhacking.miraheze.org/wiki/List_of_Hacks",
		{ waitUntil: "domcontentloaded", timeout: 300000 });
		...
		if(shouldInclude) {
			return page.evaluate(() => {
				return {
					...
					author: document.getElementsByTagName("tbody")[0]
								.children[2]
								.children[1]
								.innerText,
...
async function pokemonArchive2() {
	...
	await mainBrowserPage.goto(
		"https://www.gbahacks.com/p/pokemon-rom-hack-list.html",
		{ waitUntil: "domcontentloaded", timeout: 300000 });

	var links = await mainBrowserPage.evaluate(() => {
		var allLinks = Array.from(
			document.getElementsByClassName("item"), a => a.parentElement.href);
		// Only include ones with pages
		allLinks = allLinks.filter(
			item => item && item.indexOf("www.gbahacks.com") !== -1);
		return allLinks;
	});
				...
				if(document.querySelector("[itemprop='name']")) {
					return {
						name: document.querySelector("[itemprop='name']")
								  .innerText,
						author: document.querySelector("[itemprop='author']")
									.innerText,
						// Not the release date sadly, only the update time
						release: document.getElementsByTagName("h4")[1]
									 .innerText.replace("Updated: ", ""),
						originalgame:
							gameMapping[document.getElementsByTagName("b")[2]
											.nextSibling.textContent],
						system:
							document.querySelector("[itemprop='gamePlatform']")
								.innerText,
						downloads: null,
						type: null,
						// We can't know
					...
					return {
						name: document
							.getElementsByClassName("post-title entry-title")[0]
							.innerText,
							author: document.getElementsByClassName("pblock")[0]
								.children[14]
								.innerText,
							// Not the release date sadly, only the update time
							release: document.getElementsByTagName("h4")[1]
								.innerText.replace("Updated: ", ""),
							originalgame:
								gameMapping[document
												.getElementsByClassName(
													"pblock")[0]
												.children[8]
												.nextSibling],
							system: document.getElementsByClassName("pblock")[0]
								.children[6]
								.innerText,
							downloads: null, type: null,
							// We can't know
							important: false
					}
				}
			...
			hackEntry = await page.evaluate((gameMapping) => {
				...
				return {
					name: advanceForwards(start, 1).textContent.replace(
						...
						// Not the release date sadly, only the update time
						release: document.getElementsByTagName("h4")[1]
							.innerText.replace("Updated on- ", ""),
						originalgame: gameMapping[advanceForwards(
							start, 3).textContent.replace("\nHack of", "")],
						system: document
							.getElementsByClassName("post-title entry-title")[0]
							.innerText.split(" ")
							.pop(),
						downloads: null, type: null,
						// We can't know
						important: false, source: "gbahacks.com"
				}
			}, gameMapping);
		}
...
async function smspowerArchive1() {
	...
	var links = await mainBrowserPage.evaluate(() => {
		var allLinks = Array.from(
			document.getElementsByTagName("h3"), a => a.firstElementChild.href);
		return allLinks;
	});
	...
	await handleWebpageTemplate(links, async function returnHackEntry(page) {
		...
		return page.evaluate(() => {
			...
			var parts = document.getElementsByClassName("pagetitle")[0]
							.innerText.split(" - ");
			return {
				name: parts[1],
					author: document.getElementsByTagName("td")[5].innerText,
					release: document.getElementsByTagName("td")[4].innerText,
					originalgame: parts[0], system: systemMapping[parts[2]],
					downloads: null, type: null,
					// We can't know
					important: false, source: "smspower.org"
			}
		});
...
async function atari2600Archive() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://atariage.com/software_hacks.php?SystemID=2600&currentPage="
				+ (start - 1),
			{ waitUntil: "domcontentloaded", timeout: 300000 });

		var partialLinks = await mainBrowserPage.evaluate(() => {
			var allLinks = Array.from(
				document.getElementsByTagName("tbody")[17].children);
			allLinks
				= allLinks.slice(1, allLinks.length - 1)
					  .map(a => a.firstElementChild.firstElementChild.href);
			return allLinks;
		});
		...
		var shouldBreak = await mainBrowserPage.evaluate(() => {
			var table = document.querySelector("[valign='top'][align='left']")
							.children;
	...
	await handleWebpageTemplate(links, async function returnHackEntry(page) {
		...
		return page.evaluate(() => {
			var infoTable
				= document.querySelector("[cellspacing='0'][cellpadding='4']")
					  .firstElementChild.children;

			var temp = {
				name: document.getElementsByTagName("h1")[0].innerText.split(
					" - ")[1],
...
async function gamebananaArchive(category) {
	var indices = { projects: 11, maps: 12, skins: 13 };
	...
	while(true) {
		var projectList
			= (await axios.get("https://gamebanana.com/" + category
							   + "?vl[page]=" + start + "&mid=SubmissionsList&/"
							   + category + "=&api=SubmissionsListModule"))
				  .data;

		var partialLinks
			= projectList._aCellValues
				  .filter(project => !(
							  project._aGame._sName === "GameBanana"
							  || project._aGame._sName === "Steam"
							  || project._aGame._sName === "Unity 3D"
							  || project._aGame._sName === "Unreal Engine 4"))
				  .map(project => "https://gamebanana.com/" + category + "/"
								  + project._idItemRow
								  + "?api=StructuredDataModule");
		...
		// 1 second is risky
		await sleepForTime(1 * 1000);
	}

	console.log("Gamebanana " + category + " Length: " + links.length);
	console.log(links);

	await handleWebpageTemplate(
		links, async function returnHackEntry(jsondata, link) {
			await sleepForTime(20 * 1000);

			if(typeof jsondata !== "string") {
				var id = link.split("?")[0].replace(
					"https://gamebanana.com/" + category + "/", "");
				try {
					// This module is often not available
					var downloadData
						= (await axios.get("https://gamebanana.com/" + category
										   + "/download/" + id
										   + "?api=ArchiveTreesModule"))
							  .data;
				} catch(e) {
					downloadData = null;
				}
				var temp = {
					...
					// downloads: typeof statsData._aCellValues._nDownloadCount
					// !== "undefined" ? statsData._aCellValues._nDownloadCount
					// : null,
					downloads:
						downloadData
							? Object
								  .values(downloadData._aCellValues._aFiles)[0]
								  ._nDownloadCount
							: null,
					// type: (await axios.get("https://gamebanana.com/" +
					// category + "/" + id +
					// "?api=CategoryModule")).data._aCellValues._aCategory._sName,
...
async function moddbModsArchive() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.moddb.com/mods/page/" + start + "#modsbrowse",
			{ waitUntil: "domcontentloaded", timeout: 300000 });

		var partialLinks = await mainBrowserPage.evaluate(() => {
			// Some addons are bundled with mods, we don't need that
			var allLinks
				= Array.from(document.getElementsByClassName("rowcontent"))
					  .map(project => project.children[1]
										  .children[1]
		...
		var shouldBreak = await mainBrowserPage.evaluate(() => {
			var parts
				= document.getElementsByClassName("heading")[0].innerText.split(
					" ");
	...
	await handleWebpageTemplate(
		links, async function returnHackEntry(page, link) {
			await sleepForTime(60 * 1000);

			var part = await page.evaluate(() => {
				var rowList
					= Array.from(document.getElementsByClassName("row clear"));

				var release = null;
				var releaseElement
					= rowList
						  .filter(item => item.firstElementChild.innerText
										  === "Release date")[0]
						  .children[1]
						  .firstElementChild;
				...
				var typeString = Array
									 .from(document.getElementById("tagsform")
											   .children[1]
											   .firstElementChild.children)
									 .slice(1)
				...
				var temp = {
					name: document.getElementsByClassName("title")[0]
...
async function moddbAddonsArchive() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.moddb.com/addons/page/" + start + "#addonsbrowse",
			{ waitUntil: "domcontentloaded", timeout: 300000 });
			...
			var allLinks
				= Array.from(document.getElementsByClassName("rowcontent"))
					  .map(project => project.children[1]
										  .children[1]
										  .firstElementChild.href)
					  .filter(link => link && !link.includes("/mods/"))
			return allLinks;
		});
			...
			var part = await page.evaluate(() => {
				...
				var temp = {
					...
					author: rowList
								...
								.innerText,
					release: release === "TBD" ? null : release,
					originalgame: document.getElementsByClassName("title")[0]
									  .firstElementChild.innerText,
					system: rowList
								.filter(item => item.firstElementChild.innerText
												=== "Platforms")[0]
...
async function brawlVaultArchive() {
	...
	var lastPage = await mainBrowserPage.evaluate(() => {
		var pagesArray = document.getElementsByClassName("hackPages")[0]
							 .children[0]
							 .children;
		return parseInt(document.getElementsByClassName("hackPages")[0]
							.children[0]
	...
	for(let page = start; page <= (end === -1 ? lastPage : end); page++) {
		var pageLink
			= "http://forums.kc-mm.com/Gallery/BrawlView.php?MainType=Pack&action=changePage&Page="
			  + page + "&timeFrame=-1";
		console.log(pageLink);
		await mainBrowserPage.goto(
			pageLink, { waitUntil: "domcontentloaded", timeout: 300000 });
		try {
			var tempData = await mainBrowserPage.evaluate(() => {
				...
				Array.from(hackList).forEach(function(hackContainer) {
					...
					while(readAuthors) {
						if(hackContainer.children[authorIndex].tagName
							=== "BR") {
							readAuthors = false;
						} else {
							authors.push(
								hackContainer.children[authorIndex].innerText);
							authorIndex++;
						}
					}
					...
					tempData.push({
						...
						author: authors.join(", "),
						release: hackContainer.children[0]
									 .children[2]
									 .nextSibling.textContent.trimLeft(),
						originalgame: "Super Smash Bros Brawl",
						system: "Wii",
						downloads:
							parseInt(hackContainer.children[0]
										 .children[0]
										 .nextSibling.nextSibling.innerText),
						type: hackContainer.children[authors.length + 4]
								  .innerText.slice(1, -1),
...
async function quakeWikiArchive() {
	...
	var links = await mainBrowserPage.evaluate(() => {
		var allLinks
			= Array
				  .from(document.getElementsByClassName("w3p-subpages")[0]
							.children)
				  .map(project => project.firstChild.href);
		return allLinks;
	});
		...
		links, async function returnHackEntry(page, link) {
			...
			var temp = await page.evaluate(() => {
				...
				if(downloadLinks.length !== 0) {
					...
					var startDownloadTextSnip
						= document.getElementsByClassName("download-link")[0]
							  .innerText.indexOf("(")
						  + 1;
					downloads
						= parseInt(downloadText.slice(startDownloadTextSnip, -1)
									   .replace(" downloads", ""));
				}

				return {
					name: document.getElementsByClassName("post-title")[0]
							  .innerText,
					author: document.getElementsByClassName("post-info")[0]
								.innerText.replace("Posted by ", "")
								.split(" ")[0],
					release: document.getElementsByClassName("block-content")[0]
								 .children[0]
								 .children[2]
								 .innerText.replace("Released: ", ""),
...
async function nexusModsArchive() {
	...
	console.log(link);
	await mainBrowserPage.goto(
		link, { waitUntil: "networkidle2", timeout: 300000 });

	var start = config.ranges[18].start_page;
	var end   = config.ranges[18].end_page;

	while(true) {
		await mainBrowserPage.evaluate(
			(page) => { window.RH_ModList.Send('page', page.toString()); },
			start);

		await mainBrowserPage.waitFor(100);
		await mainBrowserPage.waitForSelector(
			"loading-wheel", { hidden: true });
		...
		try {
			var tempData = await mainBrowserPage.evaluate(() => {
				var tempData = [];
				var hackList
					= document.getElementsByClassName("tiles")[0].children;
				Array.from(hackList).forEach(function(hackContainer) {
					var container = hackContainer.children[1];

					var downloadsString = container.children[3]
					...
					if(downloadsString === "--") {
						...
						downloadNumber
							= parseFloat(downloadsString.slice(0, -1)) * 1000;
					} else if(downloadsString.endsWith("M")) {
						downloadNumber
							= parseFloat(downloadsString.slice(0, -1))
							  * 1000000;
					} else {
					...
					tempData.push({
						name: container.children[2]
								  .children[1]
								  .children[0]
								  .innerText,
						author: container.children[2]
									.children[1]
									.children[1]
									.children[3]
									.innerText.replace("Author: ", ""),
						release: container.children[2]
									 .children[1]
									 .children[1]
									 .children[1]
									 .dateTime,
						originalgame: container.children[2]
										  .children[1]
										  .children[1]
										  .children[0]
										  .children[0]
						...
						type: container.children[2]
								  .children[1]
								  .children[1]
								  .children[0]
								  .children[1]
								  .innerText,
						url: container.children[2]
								 .children[1]
								 .children[0]
								 .firstChild.href,
						source: "nexusmods"
		...
		var canFlip = await mainBrowserPage.evaluate(() => {
			var nextPage = document
							   .getElementsByClassName(
								   "page-selected mfp-prevent-close")[0]
...
async function curseforgeArchive(type) {
	...
	var urlMatcher = {
		"mc-mod": [
			"https://www.curseforge.com/minecraft/mc-mods", "Minecraft", "PC",
			19
		],
		"mc-plugin": [
			"https://www.curseforge.com/minecraft/bukkit-plugins", "Minecraft",
			"PC", 20
		],
		"wow": [
			"https://www.curseforge.com/wow/addons", "World of Warcraft", "PC",
			21
		],
		"sc2":
			["https://www.curseforge.com/sc2/assets", "StarCraft II", "PC", 22],
		"ksp": [
			"https://www.curseforge.com/kerbal/ksp-mods",
			"Kerbal Space Program", "PC", 23
		],
		"ws": [
			"https://www.curseforge.com/wildstar/ws-addons", "WildStar", "PC",
			24
		],
		"terraria":
			["https://www.curseforge.com/terraria/maps", "Terraria", "PC", 25],
		"wot": [
			"https://www.curseforge.com/worldoftanks/wot-mods",
			"World of Tanks", "PC", 26
		],
		"rom": [
			"https://www.curseforge.com/rom/addons", "Runes of Magic", "PC", 27
		],
		"rift": ["https://www.curseforge.com/rift/addons", "Rift", "PC", 28],
		"skyrim":
			["https://www.curseforge.com/skyrim/mods", "Skyrim", "PC", 29],
		"tsw": [
			"https://www.curseforge.com/tsw/tsw-mods", "The Secret World", "PC",
			30
		],
		"teso": [
			"https://www.curseforge.com/teso/teso-addons",
			"The Elder Scrolls Online", "PC", 31
		],
		"sv": [
			"https://www.curseforge.com/stardewvalley/mods", "Stardew Valley",
			"PC", 32
		],
		"swl": [
			"https://www.curseforge.com/swlegends/tswl-mods",
			"Secret World Legends", "PC", 33
		],
		"coa": [
			"https://www.curseforge.com/chronicles-of-arcadia/addons",
			"Chronicles of Arcadia", "PC", 34
		],
		"dd": [
			"https://www.curseforge.com/darkestdungeon/dd-mods",
			"Darkest Dungeon", "PC", 35
		],
		"sm": [
			"https://www.curseforge.com/surviving-mars/mods", "Surviving Mars",
			"PC", 36
		],
		"gta5": [
			"https://www.curseforge.com/gta5/gta-v-mods", "Grand Theft Auto V",
			"PC", 37
		],
		"staxel": [
			"https://www.curseforge.com/staxel/staxel-mods", "Staxel", "PC", 38
		]
	};
	...
	await mainBrowserPage.goto(
		urlBase, { waitUntil: "domcontentloaded", timeout: 300000 });

	var lastPage = await mainBrowserPage.evaluate(() => {
		var pageListContainer
			= document.getElementsByClassName("pagination-top");
	...
	for(let page = start; page <= (end === -1 ? lastPage : end); page++) {
		...
		try {
			var tempData = await mainBrowserPage.evaluate(() => {
				var tempData = [];
				var hackList
					= document.getElementsByClassName("my-4")[0]
						  .previousElementSibling.firstElementChild.children;
				Array.from(hackList).forEach(function(hackContainer) {
					...
					var types
						= Array.from(container.children[2].children[1].children)
							  .map(function(typeImage) {
								  return typeImage.firstElementChild
									  .firstElementChild.title;
							  });

					tempData.push({
						name: container.children[0]
								  .children[1]
								  .children[0]
								  .innerText,
						author: container.children[0]
									.children[1]
									.children[2]
									.innerText,
						release: container.children[1]
									 .children[1]
									 .children[2]
									 .innerText.replace("Created ", ""),
						downloads: downloadNumber,
						type: types.join(", "),
...
async function wolfenVaultArchive() {
	...
	var links = await mainBrowserPage.evaluate(() => {
		...
		function addToTotalLinks(item) {
			allLinks = allLinks.concat(
				Array.from(item.children)
					.map(item => item.firstElementChild.href));
		...
		addToTotalLinks(document.querySelector(
			"table.auto-style17:nth-child(2) > tbody:nth-child(1) > tr:nth-child(2)"));
		addToTotalLinks(document.querySelector(
			"table.auto-style17:nth-child(4) > tbody:nth-child(1) > tr:nth-child(2)"));
		addToTotalLinks(document.querySelector(
			"table.auto-style17:nth-child(6) > tbody:nth-child(1) > tr:nth-child(2)"));
		addToTotalLinks(document.querySelector(
			"table.auto-style17:nth-child(8) > tbody:nth-child(1) > tr:nth-child(2)"));
	...
	for(var i = 0; i < links.length; i++) {
		...
		console.log(link);
		await mainBrowserPage.goto(
			link, { waitUntil: "domcontentloaded", timeout: 300000 });
		...
		try {
			var tempData = await mainBrowserPage.evaluate(() => {
				...
				var hackList
					= Array
						  .from(document.querySelectorAll(
							  ".auto-style26, .auto-style25, .auto-style32"))
						  .filter(item => !!item.firstElementChild)[0]
						  .firstElementChild.children;
				Array.from(hackList).forEach(function(hackContainer, index) {
					...
					var offset
						= !hackContainer.children[0].innerText.trim() ? 1 : 0;
					...
					tempData.push({
						...
						downloads: null,
						type: hackContainer.children[3 + offset].innerText
								  + ", "
								  + hackContainer.children[4 + offset].innerText
								  + " levels",
						source: "wolfen vault"
					});
				});
...
async function halfLifeWikiArchive() {
	...
	var links = await mainBrowserPage.evaluate(() => {
		var allLinks
			= Array
				  .from(document.getElementById("GoldSrc")
							.parentElement.nextElementSibling.children)
				  .map(item => item.firstElementChild.firstElementChild
								   ? item.firstElementChild.firstElementChild
										 .href
								   : null)
				  .filter(item => item && item.includes("half-life.fandom"));
			...
			var temp = await page.evaluate(() => {
				return {
					name: document.querySelector("[data-source='name']")
							  .innerText,
					author: document.querySelector("[data-source='developer']")
								.children[1]
								.innerText.replace("\n", ", "),
					release: document.querySelector("[data-source='date']")
								 .children[1]
								 .innerText,
					// Embarrasing
					originalgame: null,
					system: document.querySelector("[data-source='platform']")
								.children[1]
								.innerText.replace("wikipedia:", ""),
					type: document.querySelector("[data-source='genre']")
								  .children[1]
								  .innerText
							  + ", "
							  + document.querySelector("[data-source='input']")
									.children[1]
									.innerText
							  + ", "
							  + document.querySelector("[data-source='mode']")
									.children[1]
									.innerText
							  + ", "
							  + document.querySelector("[data-source='engine']")
									.children[1]
									.innerText,
					important: false,
					downloads: null,
...
async function runthinkshootliveArchive(type) {
	var urlMatcher = {
		...
		"of": [
			"https://www.runthinkshootlive.com/of", "Opposing Force", "PC", 42
		],
		"hl2":
			["https://www.runthinkshootlive.com/hl2", "Half Life 2", "PC", 43],
		"ep1": [
			"https://www.runthinkshootlive.com/ep1", "Half Life 2, Episode 1",
			"PC", 44
		],
		"ep2": [
			"https://www.runthinkshootlive.com/ep2", "Half Life 2, Episode 2",
			"PC", 45
		],
	...
	await mainBrowserPage.goto(
		urlBase, { waitUntil: "networkidle2", timeout: 300000 });
	...
	var lastPage = await mainBrowserPage.evaluate(() => {
		var pageListContainer
			= document
				  .getElementsByClassName(
					  "dataTables_paginate paging_simple_numbers")[0]
				  .children[1]
				  .children;
		return parseInt(
			pageListContainer[pageListContainer.length - 1].innerText);
	});
	...
	for(let page = 1; page <= lastPage; page++) {
		try {
			var tempData = await mainBrowserPage.evaluate(() => {
				var tempData = [];
				var hackList
					= document.getElementsByClassName("display dataTable")[0]
						  .children[2]
						  .children;
				Array.from(hackList).forEach(function(hackContainer) {
					...
					tempData.push({
						...
						release: date,
						downloads:
							parseInt(hackContainer.children[5].innerText),
						type: hackContainer.children[10].innerText + ", "
								  + hackContainer.children[11].innerText + ", "
								  + hackContainer.children[12].innerText,
...
async function gta5Archive() {
	...
	while(true) {
		await mainBrowserPage.goto(
			"https://www.gta5-mods.com/all/most-downloaded/" + start,
			{ waitUntil: "domcontentloaded", timeout: 300000 });

		var partialLinks = await mainBrowserPage.evaluate(
			() => { return Array
					.from(document.getElementsByClassName("file-list")[0]
							  .firstElementChild.children)
					.map(item => item.firstElementChild.firstElementChild
									 .href) });
	...
	await handleWebpageTemplate(
		links, async function returnHackEntry(page, link) {
			var part = await page.evaluate(() => {
				return {
					name: document.getElementsByClassName("clearfix")[1]
						.innerText.trim(),
						author: document.getElementsByClassName("username")[0]
							.innerText,
						release: document.getElementById("file-dates")
							.children[1]
							.innerText.replace("\nFirst Uploaded:\n", "")
							.replace("\n", ""),
						originalgame: "GTA V", system: "PC",
						type: document.getElementById("tag-list")
							.innerText.trim()
							.replace(/\s+/g, ", "),
						important: false,
						downloads: parseInt(
							document
								.getElementsByClassName(
									"file-stat file-downloads pull-left")[0]
...
function dumpCurrentData() {
	allHackEntries.forEach((input) => {
		var data = [
			...
			input.release ? (input.release.isValid()
								 ? input.release.format("MMMM Do YYYY")
								 : "")
						  : "",
			input.release ? input.release.valueOf() || "" : "",
			input.originalgame
				? '"' + input.originalgame.replace(/\"/g, "'") + '"'
				: "",
...
(async () => {
	...
	browser = await puppeteer.launch({
		...
		args: [
			...
			"--disable-accelerated-2d-canvas", "--disable-gp",
	...
	csvWriter = fs.createWriteStream(name, { flags: "w" });

	csvWriter.write(
		"Name,Author,Release,Release (UNIX Timestamp),Original Game,System,Downloads,Type,Important,Url,Source\n");
	...
	if(config.included_scrapers.includes(0)) {
		...
		additionalHacks.forEach(function(hack) {
			// Correct date
			hack.release
				= moment(hack.release, ["DDMMYYYY", "MMMMDDYYYY", "YYYY"]);
		});
